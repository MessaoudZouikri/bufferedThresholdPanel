...
library(shinyjs)
suppressPackageStartupMessages(library(bufferedThresholdPanel))

# ── Logging ─────────────────────────────────────────────────────────────────
log_file <- file.path(tempdir(), "btpd_app.log")
log_msg <- function(level, msg) {
  cat(sprintf("[%s] %s: %s\n", format(Sys.time()), level, msg),
...
dataPreviewUI <- function(id) {
  ...
  tabsetPanel(
    tabPanel("Dataset",
      br(),
      DTOutput(ns("data_preview")) |> withSpinner(type = 4, color = "#2c5282")
    ),
    tabPanel("Descriptive Statistics",
      br(),
      DTOutput(ns("desc_stats")) |> withSpinner(type = 4, color = "#2c5282")
    ),
    tabPanel("Panel Structure",
      br(),
      plotlyOutput(ns("panel_overview"), height = "450px") |> withSpinner(type = 4, color = "#2c5282")
    )
  )
}
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$desc_stats <- renderDT({
      ...
      stats_tbl <- data.frame(
        ...
        N        = colSums(!is.na(df_num)),
        Mean     = round(colMeans(df_num, na.rm = TRUE), 4),
        SD       = round(apply(df_num, 2, sd, na.rm = TRUE), 4),
        Min      = round(apply(df_num, 2, min, na.rm = TRUE), 4),
        Q25      = round(apply(df_num, 2, quantile, 0.25, na.rm = TRUE), 4),
        Median   = round(apply(df_num, 2, median, na.rm = TRUE), 4),
        Q75      = round(apply(df_num, 2, quantile, 0.75, na.rm = TRUE), 4),
        Max      = round(apply(df_num, 2, max, na.rm = TRUE), 4),
        row.names = NULL
      )
      datatable(stats_tbl,
    ...
    output$panel_overview <- renderPlotly({
      ...
      df_agg <- df |>
        ...
        dplyr::summarise(
          dep_mean = mean(.data[[dep_nm]], na.rm = TRUE),
          q_mean   = mean(.data[[q_nm]],   na.rm = TRUE),
          .groups  = "drop"
        )
      ...
      plot_ly(df_agg) |>
        add_lines(x = ~period, y = ~dep_mean,
                  name  = paste("Mean", dep_nm),
                  yaxis = "y",
                  line  = list(color = "#2c5282", width = 2)) |>
        add_lines(x = ~period, y = ~q_mean,
                  name  = paste("Mean", q_nm),
                  yaxis = "y2",
                  line  = list(color = "#d97706", width = 2, dash = "dot")) |>
        layout(
          title  = "Panel Means Over Time",
          xaxis  = list(title = period),
          yaxis  = list(title = dep_nm, side = "left"),
          yaxis2 = list(title = q_nm,  side = "right",
                        overlaying = "y", showgrid = FALSE),
          legend = list(orientation = "h", y = -0.15)
        )
    }) |> bindCache(data_df(), dep_var(), q_var(), time_var())
  })
}
...
modelResultsUI <- function(id) {
  ...
  tagList(
    ...
    card(
      ...
      card_body(plotlyOutput(ns("forest_plot"), height = "400px") |> withSpinner(type = 4, color = "#2c5282"))
    ),
    card(
...
seqTestUI <- function(id) {
  ...
  tagList(
    ...
    card(card_header("Bootstrap Null Distribution (F1,2)"),
         card_body(plotlyOutput(ns("lr_null_plot"), height = "320px") |> withSpinner(type = 4, color = "#2c5282")))
  )
}
...
regimeAnalysisUI <- function(id) {
  ...
  tagList(
    card(
      ...
      card_body(plotlyOutput(ns("regime_plot"), height = "480px") |> withSpinner(type = 4, color = "#2c5282"))
    ),
    card(
      card_header("Dependent Variable vs Threshold Variable by Regime"),
      card_body(plotlyOutput(ns("dep_vs_q_plot"), height = "380px") |> withSpinner(type = 4, color = "#2c5282"))
    ),
    card(
      card_header("Regime Composition Table"),
      card_body(DTOutput(ns("regime_table")) |> withSpinner(type = 4, color = "#2c5282"))
    ),
    card(
      card_header(
        "Regime Matrix (Units × Time Periods)",
        div(class = "float-end",
            uiOutput(ns("name_col_selector")))
      ),
      card_body(DTOutput(ns("regime_matrix")) |> withSpinner(type = 4, color = "#2c5282"))
    )
  )
}
...
regimeAnalysisServer <- function(id, model, data_df,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$regime_plot <- renderPlotly({
      ...
      df_plot <- data.frame(
        x_val  = if (use_time) df[[time_var()]][seq_len(n_use)] else seq_len(n_use),
        q_val  = df[[q_var()]][seq_len(n_use)],
    ...
    output$regime_table <- renderDT({
      ...
      df    <- data_df()
      n_use <- model()$n_obs
      keep  <- intersect(c(id_var(), time_var(), dep_var(), q_var()), names(df))
      df_aug <- cbind(
        df[seq_len(n_use), keep, drop = FALSE],
    ...
    output$name_col_selector <- renderUI({
      req(data_df())
      df      <- data_df()
      used    <- c(id_var(), time_var(), dep_var(), q_var())
      chr_cols <- names(df)[vapply(df, function(x) is.character(x) || is.factor(x), logical(1L))]
      avail   <- setdiff(chr_cols, used)
      if (length(avail) == 0L) return(NULL)
      selectInput(session$ns("name_col"),
                  label    = NULL,
                  choices  = c("ID only" = "", setNames(avail, avail)),
                  selected = "",
                  width    = "160px")
    })

    output$regime_matrix <- renderDT({
      req(model(), data_df())
      df    <- data_df()
      n_use <- model()$n_obs

      df_long <- data.frame(
        unit   = as.character(df[[id_var()]][seq_len(n_use)]),
        period = df[[time_var()]][seq_len(n_use)],
        Regime = model()$regime_classification,
        stringsAsFactors = FALSE
      )

      # Wide pivot: rows = units, columns = time periods
      df_wide <- reshape(df_long, idvar = "unit", timevar = "period",
                         direction = "wide", v.names = "Regime")
      names(df_wide) <- sub("^Regime\\.", "", names(df_wide))
      names(df_wide)[names(df_wide) == "unit"] <- id_var()

      # Sort numerically when id is all-numeric, otherwise alphabetically
      id_vals <- df_wide[[id_var()]]
      num_ids <- suppressWarnings(as.numeric(id_vals))
      df_wide <- if (!anyNA(num_ids)) df_wide[order(num_ids), ] else df_wide[order(id_vals), ]
      rownames(df_wide) <- NULL

      # Optionally replace numeric id with a name column
      name_col <- if (!is.null(input$name_col) && nzchar(input$name_col)) input$name_col else NULL
      if (!is.null(name_col)) {
        lookup <- unique(data.frame(
          unit = as.character(df[[id_var()]][seq_len(n_use)]),
          name = as.character(df[[name_col]][seq_len(n_use)]),
          stringsAsFactors = FALSE
        ))
        df_wide[[id_var()]] <- lookup$name[match(df_wide[[id_var()]], lookup$unit)]
        names(df_wide)[names(df_wide) == id_var()] <- name_col
        first_col <- name_col
      } else {
        first_col <- id_var()
      }

      time_cols  <- setdiff(names(df_wide), first_col)
      n_reg      <- model()$n_regimes
      reg_colors <- c("1" = "#d4edda", "2" = "#cce5ff", "3" = "#fff3cd")
      ...
      dt <- datatable(df_wide,
                      ...
                      caption  = htmltools::tags$caption(
                        ...
                        paste("Regime per unit and time period.",
                              paste(paste0("Regime ", seq_len(n_reg),
                                           " = colour ", seq_len(n_reg)),
                                    collapse = "; "))
                      ))
      ...
      for (col in time_cols) {
        dt <- dt |> formatStyle(
          col,
          backgroundColor = styleEqual(seq_len(n_reg),
                                        reg_colors[as.character(seq_len(n_reg))]),
          textAlign = "center"
        )
      }
      dt
...
bootstrapUI <- function(id) {
  ...
  tagList(
    card(
      ...
      card_body(plotlyOutput(ns("bootstrap_plot"), height = "380px") |> withSpinner(type = 4, color = "#2c5282"))
    ),
    card(
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$comparison_table <- render_gt({
      ...
      make_row <- function(m, label) {
        k   <- length(m$thresholds) + length(as.vector(m$coefficients))
        dev <- m$n_obs * log(m$ssr / m$n_obs)
        data.frame(
          Model      = label,
          ...
          AIC_approx = round(dev + 2 * k, 2),
          BIC_approx = round(dev + log(m$n_obs) * k, 2),
      ...
      gt::gt(tbl) |>
        ...
        gt::tab_footnote("Lower SSR, AIC, and BIC indicate better fit. BIC penalises model complexity more heavily than AIC.")
    })
  })
}
  ...
  tagList(
    layout_columns(
      ...
      card(card_header("Residuals vs Fitted"),
           card_body(plotlyOutput(ns("diag_resid_fitted"), height = "350px") |> withSpinner(type = 4, color = "#2c5282"))),
      card(card_header("Normal Q-Q Plot"),
           card_body(plotlyOutput(ns("diag_qq"),           height = "350px") |> withSpinner(type = 4, color = "#2c5282")))
    ),
    layout_columns(
      col_widths = c(6, 6),
      card(card_header("Residual Distribution"),
           card_body(plotlyOutput(ns("diag_hist"), height = "350px") |> withSpinner(type = 4, color = "#2c5282"))),
      card(card_header("Residual ACF"),
           card_body(plotlyOutput(ns("diag_acf"),  height = "350px") |> withSpinner(type = 4, color = "#2c5282")))
    )
  )
}
      ...
      card(
        ...
        card_body(
          ...
          selectInput("n_thresh",
                      ...
                      selected = 1),
          tooltip(
            radioButtons("grid_mode", "Grid Search",
              choices = list(
                "Exhaustive (all unique q values)" = "exhaustive",
                "Capped (faster for bootstraps)"   = "capped"),
              selected = "exhaustive"),
            "Exhaustive guarantees the global minimum. Cap speeds up bootstrap loops."
          ),
          conditionalPanel(
            condition = "input.grid_mode == 'capped'",
          ...
          conditionalPanel(
            ...
            tooltip(
              ...
              "3-regime BTPD searches 4 parameters; keep low (< 50) for speed."
            )
          ),
          selectInput("se_type", "Robust SE Type",
                      choices  = c("HC3", "HC2", "HC1", "HC0"),
                      selected = "HC3"),
          uiOutput("se_type_desc"),
          tooltip(
            sliderInput("trim_frac", "Trimming Fraction",
                        min = 0.05, max = 0.30, value = 0.15, step = 0.05),
            "Fraction trimmed from each tail of the threshold variable (Hansen 1999)."
          )
        )
      ),
...
server <- function(input, output, session) {
  ...
  r_grid_size   <- reactive({
    if (input$grid_mode == "exhaustive") NULL else input$grid_size
  })
  ...
  # ── SE type description ──────────────────────────────────────────────────
  output$se_type_desc <- renderUI({
    desc <- switch(input$se_type,
      "HC0" = "Basic sandwich estimator. No degrees-of-freedom correction; can underestimate standard errors in small panels.",
      "HC1" = "HC0 with a degrees-of-freedom correction (n ÷ (n−k)). Slightly more conservative than HC0; common in econometrics software.",
      "HC2" = "Leverage-adjusted estimator. Scales each squared residual by 1÷(1−hᵢᵢ), where hᵢᵢ is the leverage of observation i. More reliable than HC0/HC1 in small samples.",
      "HC3" = "Recommended for small samples. Most conservative: uses 1÷(1−hᵢᵢ)², giving larger standard errors near influential observations and reducing small-sample bias."
    )
    helpText(desc)
  })
  ...
  observeEvent(input$data_file, {
    ...
    if (!ext %in% c("csv", "xlsx", "xls")) {
      showNotification("Unsupported format. Please use CSV or XLSX.", type = "error")
      return()
    }
    if (file.size(input$data_file$datapath) > 100 * 1024^2) {
      showNotification("File too large (max 100 MB).", type = "error")
      return()
    }
    tryCatch({
      rv$data <- switch(ext,
      ...
      if (nrow(rv$data) < 20L)
        stop("Dataset must have at least 20 observations.")
      if (ncol(rv$data) < 4L)
        stop("Dataset must have at least 4 columns (id, time, dependent, threshold).")
      populate_vars(rv$data)
      log_msg("INFO", sprintf("Data loaded: %s (%d x %d)",
        input$data_file$name, nrow(rv$data), ncol(rv$data)))
      showNotification("Data loaded successfully!", type = "message")
    }, error = function(e) {
      log_msg("ERROR", sprintf("File load failed: %s", e$message))
  ...
  observeEvent(input$run_model, {
    ...
    on.exit(shinyjs::enable("run_model"))
    ...
    withProgress(message = "Estimating model...", value = 0, {
      tryCatch({
        ...
        gs    <- if (input$grid_mode == "exhaustive") NULL else input$grid_size
  ...
  output$dl_results <- downloadHandler(
    ...
    content  = function(file) {
      ...
      if (!requireNamespace("writexl", quietly = TRUE)) {
        showNotification(
          "Package 'writexl' required for XLSX export. Install with: install.packages('writexl')",
