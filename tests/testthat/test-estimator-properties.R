...
test_that("PTR recovers the true threshold to within grid resolution", {
  ...
  fit <- bptr(y ~ x1, data = df, id = "id", time = "time", q = "q",
              n_thresh = 1L, buffer = FALSE)
  # Exhaustive grid over all observed q values (Hansen 1999, §3) guarantees
  # the global SSR minimum; with N=400 the grid has ~320 points, giving a
  # resolution of ~0.01.  Allow ±0.05 to absorb grid quantisation.
...
test_that("estimated PTR threshold minimises SSR over the entire exhaustive grid", {
  ...
  q_lo  <- as.numeric(quantile(df$q, 0.15, type = 7L, na.rm = TRUE))
  q_hi  <- as.numeric(quantile(df$q, 0.85, type = 7L, na.rm = TRUE))
