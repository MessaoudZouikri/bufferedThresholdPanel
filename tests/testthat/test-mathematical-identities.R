...
test_that("buildBufferIndicators3() first-obs tiebreaker uses < (strict upper)", {
  # Boundaries chosen so buffer midpoints give EXACTLY equal distances in
  # IEEE 754 double precision, testing the < vs <= distinction.
  rL1 <- -2; rU1 <- 0; rL2 <- 2; rU2 <- 4
  ids <- c(1L, 2L, 3L, 4L)
  qs  <- c(-1,   # buf1 midpoint, exactly eq → < gives R2 (upper)
            1,   # clear R2
            3,   # buf2 midpoint, exactly eq → < gives R3 (upper)
            5)   # clear R3
  d <- buildBufferIndicators3(qs, rL1, rU1, rL2, rU2, ids)
  expect_equal(d[1L], 2L, label = "buf1 equidistant → R2 (upper)")
  expect_equal(d[2L], 2L, label = "clear R2 → R2")
  expect_equal(d[3L], 3L, label = "buf2 equidistant → R3 (upper)")
  expect_equal(d[4L], 3L, label = "clear R3 → R3")
})
