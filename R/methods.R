...
#'   i.e. 15\%--85\% range, following Hansen (1999) and Hansen (2000))
...
#'   The RNG state is saved before and restored after, so there is no
#'   persistent side effect on the global \code{.Random.seed}.
...
#'   \code{bptr} object)
#' @seealso \code{\link{bptr_test_seq}} for the full sequential testing
#'   procedure (F1,2 followed by F2,3);
#'   \code{\link{bptr_test_23}} for the F2,3 test in isolation;
#'   \code{\link{bptr}} for model estimation.
#' @references
...
bptr_test <- function(formula, data, id, time, q,
                      ...
                      trim = 0.15, grid_size = NULL, seed = 42) {
  ...
  # Pre-compute unit→row index map once; avoids O(N²T) scanning per iteration
  unit_rows <- split(seq_along(id_vec), id_vec)

  boot_stats <- vapply(seq_len(n_boot), function(b) {
    rs <- numeric(length(resid0))
    sampled_ids <- sample(uniq, replace = TRUE)
    for (j in seq_along(uniq)) {
      orig_idx <- unit_rows[[as.character(uniq[j])]]
      src_idx  <- unit_rows[[as.character(sampled_ids[j])]]
      rs[orig_idx] <- resid0[src_idx]
    }
...
print.bptr_test <- function(x, ...) {
  ...
  cat(sprintf("Conclusion: %s threshold effect %s\n\n",
              if (x$p_value < .10) "Significant" else "Not significant", sig))
  invisible(x)
}
...
#' @param trim Numeric. Trimming fraction (default 0.15, i.e. 15\%--85\% range,
#'   following Hansen (1999) and Hansen (2000))
#' @param grid_size Integer or \code{NULL}. Optional cap on the number of grid
...
#' @param grid_size Integer or \code{NULL}.  Number of candidate threshold
#'   values to search per bootstrap replication.  \code{NULL} (default)
#'   re-uses the grid from the original fit (exhaustive over all unique
#'   observed values in the trimming range) — maximum precision.  Pass an
#'   integer (e.g. \code{50}) to cap the search for faster but approximate
#'   bootstrap CIs during exploration.
#' @param workers Integer or \code{NULL}. Parallel workers for
#'   \code{future}/\code{furrr}.  Defaults to \code{availableCores() - 1}
#'   in interactive use; defaults to \code{1} (sequential) when run inside
#'   \code{testthat} (package is loaded via \code{load_all()} rather than
#'   installed, avoiding multisession-attach failures).
#' @param seed Integer. Random seed (default 42).
...
#' print(boot)
#' }
bptr_bootstrap <- function(x, n_boot = 299, grid_size = NULL, workers = NULL, seed = 42) {
  if (!inherits(x, "bptr")) stop("Object must be of class 'bptr'")
  if (x$n_thresh != 1L)
    stop("bptr_bootstrap() is implemented for 2-regime (n_thresh = 1) models only. ",
         "A 3-regime version (bootstrapping beta3 and all four buffer parameters) is not yet available.")
  if (exists(".Random.seed", envir = .GlobalEnv, inherits = FALSE)) {
    old_seed <- .Random.seed
    on.exit(.Random.seed <<- old_seed, add = TRUE)
  } else {
  ...
  w <- if (is.null(workers)) {
    if (nzchar(Sys.getenv("TESTTHAT"))) 1L else
  ...
  # Pre-compute unit→row index map once; avoids O(N²T) scanning per iteration
  unit_rows <- split(seq_along(id_v), id_v)
  ...
  one_boot <- function(b) {
    ...
    tryCatch(
      { fb <- bptr(formula = x$formula, data = db, id = x$id, time = x$time,
                   ...
                   trim = x$trim, grid_size = grid_size, se_type = x$se_type)
