#' @useDynLib bufferedThresholdPanel, .registration = TRUE
...
#'
#' @seealso
#'   \code{\link{bptr_test}} and \code{\link{bptr_test_seq}} for sequential
#'   bootstrap tests of regime number;
#'   \code{\link{bptr_bootstrap}} for bootstrap confidence intervals on the
#'   threshold parameter (2-regime models);
#'   \code{\link[broom]{tidy}}, \code{\link[broom]{glance}},
#'   \code{\link[broom]{augment}} for tidy model summaries;
#'   \code{\link{bptr_table}} for publication-ready tables;
...
bptr <- function(formula, data, id, time, q,
  ...
  if (!is.character(id)   || length(id)   != 1L) stop("'id' must be a single character string")
  if (!is.character(time) || length(time) != 1L) stop("'time' must be a single character string")
  if (!is.character(q)    || length(q)    != 1L) stop("'q' must be a single character string")
  if (!q %in% names(data))           stop(paste("Threshold variable", q, "not found in data"))
  if (!se_type %in% c("HC0", "HC1", "HC2", "HC3"))
    stop("'se_type' must be one of \"HC0\", \"HC1\", \"HC2\", or \"HC3\"")
  if (!is.null(grid_size)) {
    gs_check <- suppressWarnings(as.integer(grid_size))
    if (is.na(gs_check) || gs_check < 1L)
      stop("'grid_size' must be a positive integer when provided")
  }
  if (!is.null(grid_size_3)) {
    gs3_check <- suppressWarnings(as.integer(grid_size_3))
    if (is.na(gs3_check) || gs3_check < 1L)
      stop("'grid_size_3' must be a positive integer when provided")
  }
  ...
  time_lev <- sort(unique(data[[time]]))
  id_lev   <- sort(unique(data[[id]]))
  regime_table <- as.data.frame(
    tapply(regime_classification,
           list(factor(data[[id]], levels = id_lev),
                factor(data[[time]], levels = time_lev)),
           identity)
  )
