...
```r
# From CRAN (once published) — no compiler needed
install.packages("bufferedThresholdPanel")

# Development version from GitHub — requires a C++ compiler
remotes::install_github("MessaoudZouikri/bufferedThresholdPanel")
```

**Compiler requirement**: The package uses Rcpp for performance. Installing from
CRAN provides pre-compiled binaries (no compiler needed).  Installing from
GitHub requires a C++17 compiler: [Rtools](https://cran.r-project.org/bin/windows/Rtools/)
on Windows, Xcode Command Line Tools on macOS (`xcode-select --install`), or
`build-essential` on Linux.  If you already have any Rcpp package installed,
